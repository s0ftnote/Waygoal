...
export function WaygoalTurnCanvas({ sessionId, targetVersion, board, layouts, onSaveBoard, sessions, locateEntry, inspectEntry, materials, busy, onLocate, onContinue, onOpenSession, onDismissPreview, onMaterial, onOverview, onFork, onReady }: Props) {
  ...
  useEffect(() => { positions.current = Object.fromEntries(cards.map(card => [card.key, card.position])); }, [cards]);
  ...
  const persistLayout = (next: WaygoalTurnLayout) => {
    const saved = { ...next, positions: { ...positions.current, ...next.positions } };
  ...
  const fittedCamera = (width: number, height: number) => {
    const points = [...cards.map(card => card.position), next];
  ...
  useEffect(() => {
    ...
    if (activeKey && (previous?.session !== sessionId || count > previous.count)) focusCard(activeKey, previous?.session !== sessionId);
  ...
  }, [locateEntry, cards, focusCard, sessionId]);
  ...
  return <section className="waygoal-turn-canvas" aria-label="轮次画布">
    <div className="waygoal-turn-toolbar">
      <button type="button" aria-label="当前轮次" onClick={() => { setActionKey(null); if (active) focusCard(active.key, true); }}>◎ 回到正在聊的位置</button>
      <button type="button" aria-label="全景" onClick={() => { setActionKey(null); const bounds = viewport.current?.getBoundingClientRect(); if (bounds) setCamera(fittedCamera(bounds.width, bounds.height)); }}>看全局</button>
      <details className="waygoal-turn-more" ref={toolsMenu}>
        <summary aria-label="画布操作">•••</summary>
        <div className="waygoal-turn-more-body">
      <button type="button" onClick={onOverview}>会话与票据</button>
      <select aria-label="查看会话轮次" value={sourceId} onChange={event => {
        const id = event.target.value; setSourceId(id);
        const last = data[id]?.turns.findLast(turn => turn.active);
        const key = last && graph.aliases.get(turnKey(id, last.id)); if (key) focusCard(key, true);
    ...
    <div className="waygoal-turn-viewport" ref={viewport} tabIndex={0} aria-label="轮次画布：拖动空白处平移，滚轮缩放"
      onWheel={event => { const bounds = event.currentTarget.getBoundingClientRect(), x = event.clientX - bounds.left, y = event.clientY - bounds.top; const minimumScale = Math.min(.3, fittedCamera(bounds.width, bounds.height).scale); setCamera(camera => { const scale = Math.max(minimumScale, Math.min(1.5, camera.scale * (event.deltaY > 0 ? .92 : 1.08))); return { x: x - (x - camera.x) * scale / camera.scale, y: y - (y - camera.y) * scale / camera.scale, scale }; }); }}
      ...
      <div className="waygoal-turn-world" style={{ "--turn-scale": camera.scale, transform: `translate(${camera.x}px, ${camera.y}px) scale(${camera.scale})` } as CSSProperties}>
        ...
        {sessions.map(session => { const p = graph.anchors[session.id]; return p ? <div className={`waygoal-session-label${session.id === sessionId ? " current" : ""}`} key={session.id} style={{ left: p.x, top: p.y - 38 }}><span>{session.title}{session.id === sessionId ? " · 正在聊" : ""}</span>{session.id !== sessionId && <button type="button" disabled={busy} onClick={() => onOpenSession(session.id)}>在此继续</button>}</div> : null; })}
        {cards.map((card, index) => <article key={card.key} data-turn={card.turn.id} data-session={card.sessionId} data-turn-key={card.key} className={`waygoal-turn-card${activeKey === card.key ? " active" : ""}${selected === card.key ? " selected" : ""}`} style={{ left: card.position.x, top: card.position.y, width: WIDTH, height: HEIGHT }}>
          <button type="button" className="waygoal-turn-content" aria-label={`${index + 1} · ${card.turn.question}`} aria-pressed={selected === card.key}
            onPointerDown={event => { setActionKey(null); gesture.current = { id: card.key, start: { x: event.clientX, y: event.clientY }, origin: card.position, moved: false }; event.currentTarget.setPointerCapture(event.pointerId); }}
        ...
        <button type="button" className="waygoal-next-turn" data-next-turn style={{ left: next.x, top: next.y, width: WIDTH }} disabled={busy || capturing || mode !== "reference" || !from}
      ...
      {showActions && actionCard && actionMember && <div className="waygoal-turn-actions" role="group" aria-label="所选卡片操作"
        ...
        {!actionOnCurrentPath && <>
          <button type="button" disabled={busy || capturing} onClick={() => void reference(actionCard.key, "answer")}>引用回答</button>
    ...
    {inspected && <aside className="waygoal-edge-preview" aria-label="关系来源">
      ...
      <div>{[inspected.from, inspected.to].map((key, index) => <button key={index} type="button" disabled={!byKey.has(key)} onClick={() => { const card = byKey.get(key); if (card) { locate(card); focusCard(key, true); } }}>{index === 0 ? "查看来源" : "查看使用位置"}</button>)}</div>
      {!byKey.has(inspected.from) && <p>来源不在当前画布或已不可读取；已发送的原文仍保留。</p>}
