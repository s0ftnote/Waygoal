...
export function WaygoalArrange({ picked, onGroup, onLink, onClear }: Props) {
  ...
  return <div className="waygoal-arrange" data-arrange>
    <span className="waygoal-count" data-picked={picked.length}>已选 {picked.length} 个：{picked.map(card => card.title).join("、")}</span>
