...
import { applyCanvasPatch, buildSnapshot, resolveWorkspaceCwd } from "@/lib/waygoal-store";
...
export async function GET(req: Request) {
  try {
    ...
    const snapshot = buildSnapshot(cwd, mergeSessionLists(persisted, runtime), getRunningRpcSessionIds());
...
// PATCH /api/waygoal  body: { cwd, positions?, view?, lastViewed? }
// Stores layout and the last viewed node. Restoring later reads only.
export async function PATCH(req: Request) {
  try {
    ...
    const record = applyCanvasPatch(cwd, { positions: body.positions, view: body.view, lastViewed: body.lastViewed });
