...
const fs = require("fs");
// eslint-disable-next-line @typescript-eslint/no-require-imports
const { getHelpText, parseLaunchOptions } = require("./waygoal-options");
// eslint-disable-next-line @typescript-eslint/no-require-imports
...
if (!loopbackHostnames.has(hostname)) {
  if (passwordEnabled) {
    console.warn(
      `Warning: waygoal is listening on ${hostname} with password authentication over HTTP. Use HTTPS or a trusted VPN to protect the password in transit.`,
    );
  } else {
    console.warn(
      `Warning: waygoal is listening on ${hostname} without authentication. Only use this on a trusted network.`,
    );
  }
}
...
const child = spawn(process.execPath, [nextBin, ...nextArgs], {
  ...
  env: { ...process.env, WAYGOAL: "1", PI_WEB_HOSTNAME: hostname },
});
