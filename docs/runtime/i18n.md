...
already provides it in `src/app/page.tsx`.
...
key set from `src/shared/i18n/messages/en.ts` when starting a new package. Missing
keys fall back to English, but a complete package is preferred for a user
facing language.

### 2. Register The Package

Import the package and add it to `localePlugins` in `src/shared/i18n/registry.ts`:
...
`src/shared/i18n/types.ts` and update any locale validation that intentionally lists
the built-in identifiers.

### 4. Add Tests

Extend `src/shared/i18n/registry.test.mjs` when browser-language detection or locale
listing changes. Extend `src/shared/i18n/format.test.mjs` when interpolation or
