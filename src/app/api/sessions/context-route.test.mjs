...
const { buildSessionContext } = await jiti.import("@/server/sessions/session-reader");
...
test("context route parses ?tail and ?before, excluding the boundary on paging", () => {
  ...
  assert.match(routeSrc, /buildSessionContext\(sm\.getEntries\(\) as never, before \?\? leafId \?\? sm\.getLeafId\(\), \{[^}]*excludeLeaf: Boolean\(before\)/);
});
