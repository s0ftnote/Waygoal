import { renameSession } from "@/server/sessions/session-access";
...
import {
  ...
  buildSessionContext,
  readSessionHeader,
} from "@/server/sessions/session-reader";
import { sessionPathKey } from "@/server/sessions/session-path";
import { getRpcSession } from "@/server/agent/rpc-manager";
import { projectTreeForResponse } from "@/features/workspace/project-tree";
import { computeSessionTotalActiveMs } from "@/server/sessions/session-timing";
import { computeSessionStats } from "@/features/sessions/session-stats";
import type { SessionEntry } from "@/shared/types";
import { readSubagentRun, readSubagentSessionResources, SUBAGENT_META_TYPE } from "@/server/agent/subagents";
import { readSessionToolSelection } from "@/server/sessions/session-tool-selection";
...
export async function PATCH(
  ...
  try {
    ...
    if (!await renameSession(id, name)) return NextResponse.json({ error: "Session not found" }, { status: 404 });
