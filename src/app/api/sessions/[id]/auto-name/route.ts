import { nameVersion, renameSession } from "@/server/sessions/session-access";
import { NextResponse } from "next/server";
import type { AgentSession } from "@earendil-works/pi-coding-agent";
import { generateSessionTitle } from "@/server/sessions/session-title";
import { getRpcSession, startRpcSession } from "@/server/agent/rpc-manager";
import { resolveSessionPath } from "@/server/sessions/session-reader";
...
export async function POST(
  ...
  try {
    ...
    const version = nameVersion(session.inner.sessionManager);
    ...
    await renameSession(id, result.title, version);
