...
test("exact prompts are sent through before_agent_start instead of the SDK prompt state", async () => {
  ...
  assert.match(startupSource, /chatOnly \? \{ extensionFactories: \[exactSystemPromptExtension\] \} : \{\}/);
  assert.match(subagentSource, /extensionFactories: \[createExactSystemPromptExtension\(\(\) => profile\.systemPrompt\)\]/);
