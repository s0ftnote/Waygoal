import { createRecordedFork, type ForkCanvas } from "../sessions/fork-service";
import type { ThinkingLevel } from "@earendil-works/pi-agent-core";
import { createWaygoalExtension } from "./extension";
import { createAgentSessionFromServices, createAgentSessionServices, getAgentDir, initTheme, SessionManager, SettingsManager, Theme } from "@earendil-works/pi-coding-agent";
import { KeybindingsManager as TuiKeybindingsManager, TUI_KEYBINDINGS } from "@earendil-works/pi-tui";
import { randomUUID } from "crypto";
import { existsSync, realpathSync, writeFileSync } from "fs";
import { resolve } from "path";
import { validateAgentImages } from "../../features/chat/image-attachments";
import { invalidateModelsCache } from "../../features/settings/models-cache";
import { resolveVisibleModels, selectInitialModelScope } from "../models/model-scope";
import {
  createProjectCommandBashExtension,
  createProjectCommandBashOperations,
  preferUserBashExtension,
} from "./project-command-env";
import { cacheSessionPath, invalidateSessionListCache, resolveSessionPath } from "../sessions/session-reader";
import { getProjectTrustStatus, projectTrustReloadOptions } from "../workspace/project-trust";
import { persistExplicitStartupPreferences } from "./startup-preferences";
import { notifySessionComplete } from "../http/web-push";
import { hasActiveSessionLivenessProvider } from "../sessions/session-liveness";
...
import { resolveShellTools } from "../workspace/powershell-settings";
...
} from "../sessions/session-tool-selection";
...
export async function startRpcSession(
  ...
  const starting = (async () => {
    ...
    // Resolved after session creation, then read again on every run/reload.
    ...
    const services = await createAgentSessionServices({
      ...
      resourceLoaderOptions: subagentResources
        ? {
            ...
            ...(chatOnly ? { extensionFactories: [exactSystemPromptExtension] } : {}),
          }
        : chatOnly
    ...
    const hasExistingMessages = sessionManager.getBranch().some((entry) => entry.type === "message" && entry.message.role !== "system");
