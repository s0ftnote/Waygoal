...
const { createWaygoalExtension, MISSING_SKILL_NOTICE, parseSkillCommand } = await jiti.import("../agent/extension.ts");
const workspaces = await jiti.import("../workspace/workspaces.ts");
const paths = await jiti.import("../workspace/dirs.ts");
...
const { TICKET_CARD_HEIGHT } = await jiti.import("../../shared/waygoal-types.ts");
...
test("ticket discussions read the durable session lineage without a canvas origin copy", async () => {
  const s = sandbox();
  try {
    const { writeOrigin } = await jiti.import("../sessions/lineage.ts");
    localMap(s.a, "screening", { "01-feeling.md": ticketBody("感受") });
    const path = ".scratch/screening/issues/01-feeling.md";
