...
test("all project resource loaders and reloads enforce project trust", async () => {
  const rpcSource = await readFile(new URL("../agent/rpc-manager.ts", import.meta.url), "utf8");
  const modelsSource = await readFile(new URL("../../app/api/models/route.ts", import.meta.url), "utf8");
  const skillsSource = await readFile(new URL("../extensions/skills-service.ts", import.meta.url), "utf8");
