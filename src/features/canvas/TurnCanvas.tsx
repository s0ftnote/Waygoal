...
import { TakeawayEditor } from "../materials/TakeawayEditor";
import { zoomTier, takeawayChanged, type ZoomTier, type TurnTakeaway } from "@/features/materials/takeaways";
import { expandedTurns, type SessionSize } from "@/features/canvas/session-expansion";
import type { WaygoalView, WaygoalPoint, WaygoalTurnLayout } from "@/shared/waygoal-types";
import type { WaygoalTurns, WaygoalTurn } from "@/features/sessions/turns";
import { MATERIAL_SCOPES, type MaterialScope, type MaterialSnapshot } from "@/features/materials/materials";
import { findEntryTurn } from "@/features/sessions/entry-index";
import { projectTurnBoard, turnKey, TURN_WIDTH as WIDTH, TURN_HEIGHT as HEIGHT, TURN_GAP as GAP, type BoardCard, type BoardEdge, type BoardSession } from "@/features/canvas/turn-board";
import { sessionsToLoad } from "@/features/sessions/turn-loading";
import { useVisibleCanvas } from "./useVisibleCanvas";
import { intersectsView } from "./visible-scene";
...
export function WaygoalTurnCanvas({ sessionId, targetVersion, board, layouts, onSaveBoard, sessions, locateEntry, locateMaterial, inspectEntry, materials, busy, onLocate, onContinue, onOpenSession, onDismissPreview, onMaterial, onMaterials, onOverview, onFork, onReady, expanded, worldHost, camera, setCamera, onGrabCamera, setCameraDirect, onGeometry, onExpand, onFit }: Props) {
  ...
  const visibleBox = useVisibleCanvas(worldHost, camera, viewportSize);
  ...
  return <section className="waygoal-turn-canvas waygoal-turn-embedded" aria-label="轮次画布" data-selecting={selecting || undefined}>
    ...
    <div className="waygoal-turn-viewport" ref={viewport} tabIndex={0} aria-label="轮次画布：拖动空白处平移，滚轮缩放"
      ...
      onPointerUp={event => { if (gesture.current?.pointerId !== event.pointerId) return; if (gesture.current?.id && gesture.current.moved) persistLayout(layoutRef.current); setDraggedCard(null); requestAnimationFrame(() => { gesture.current = null; }); }} onPointerCancel={event => { if (gesture.current?.pointerId === event.pointerId) { gesture.current = null; setDraggedCard(null); } }}>
      {worldHost && createPortal(<div className="waygoal-turn-world" data-zoom-tier={tier} data-mode={mode} data-selecting={selecting || undefined} style={{ "--turn-scale": camera.scale } as CSSProperties}>
        <svg className="waygoal-turn-lines" width="1" height="1">
          {graph.edges.map(edge => { const a = byKey.get(edge.from), b = byKey.get(edge.to); if (edge.kind === "history" && (!a || !b) || edge.kind === "fork" && !a && !b && !edge.status) return null; const from = a?.position ?? sessions.find(session => session.id === edge.fromSession)?.position, to = b?.position ?? sessions.find(session => session.id === edge.toSession)?.position; if (!from || !to) return null; if (focusedEdge !== edge.key && inspected?.key !== edge.key && !intersectsView(visibleBox, { x: Math.min(from.x, to.x) - WIDTH, y: Math.min(from.y, to.y) - HEIGHT - GAP, width: Math.abs(to.x - from.x) + WIDTH * 3, height: Math.abs(to.y - from.y) + (HEIGHT + GAP) * 3 })) return null; const path = edgePath(from, to, edge.kind === "reference" || edge.kind === "association", !!a, !!b); return <g key={edge.key} data-spatial-edge={edge.key} data-origin-status={edge.status ?? "resolved"} data-fork-target={edge.kind === "fork" ? edge.toSession : undefined} data-spatial-from={a ? `turn:${a.key}` : `node:${edge.fromSession}`} data-spatial-to={b ? `turn:${b.key}` : `node:${edge.toSession}`}><path d={path} className={edge.kind} /><path d={path} className="edge-hit" role="button" tabIndex={0} aria-label={`${edge.kind === "reference" ? "引用材料" : edge.kind === "fork" ? "分叉来源" : edge.kind === "association" ? "手动关联" : "连续历史"}：${title(edge.fromSession)} → ${title(edge.toSession)}`} onFocus={() => setFocusedEdge(edge.key)} onBlur={() => setFocusedEdge(null)} onClick={() => inspect(edge)} onKeyDown={event => { if (event.key === "Enter" || event.key === " ") { event.preventDefault(); inspect(edge); } }} /></g>; })}
        ...
        {cards.map((card, index) => {
          const retained = card.key === focusedCard || card.key === draggedCard || card.key === actionKey || card.key === from || card.key === editingTakeaway?.key;
          if (!retained && !intersectsView(visibleBox, { ...card.position, width: WIDTH, height: HEIGHT })) return null;
          ...
          return <article key={card.key} onFocusCapture={() => setFocusedCard(card.key)} onBlurCapture={event => { if (!event.currentTarget.contains(event.relatedTarget)) setFocusedCard(null); }} data-turn={card.turn.id} data-session={card.sessionId} data-turn-key={card.key} data-material-selected={selection.includes(card.key) || undefined} data-link-source={mode === "link" && from === card.key || undefined} data-spatial-key={`turn:${card.key}`} data-spatial-owner={projection.owners.get(card.key)} data-takeaway={takeaway ? changed ? "changed" : takeaway.status : undefined} className={`waygoal-turn-card${activeKey === card.key ? " active" : ""}${selected === card.key ? " selected" : ""}`} style={{ left: card.position.x, top: card.position.y, width: WIDTH, height: HEIGHT }}>
          <button type="button" className="waygoal-turn-content" data-spatial-face aria-label={`${index + 1} · ${card.turn.question}`} aria-pressed={mode === "link" ? from === card.key : selecting ? selection.includes(card.key) : selected === card.key}
            onPointerDown={event => { if (mode === "link") { event.stopPropagation(); event.preventDefault(); event.currentTarget.focus({ preventScroll: true }); return; } if (selecting || event.metaKey || event.ctrlKey) { event.stopPropagation(); return; } if (gesture.current || event.button !== 0) return; onGrabCamera(); setDraggedCard(card.key); setActionKey(null); gesture.current = { id: card.key, start: { x: event.clientX, y: event.clientY }, origin: positions.current[card.key] ?? card.position, moved: false, pointerId: event.pointerId }; event.currentTarget.setPointerCapture(event.pointerId); }}
