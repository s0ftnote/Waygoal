...
import { isBlockingExtensionUiRequest } from "@/shared/browser-notifications";
...
import { getPreferredToolPreset, setPreferredToolPreset } from "@/features/chat/tool-preset-preference";
import { getPresetFromToolNames, getToolNamesForPreset, type ToolEntry, type ToolPreset } from "@/features/chat/tool-presets";
import type { SessionStatsInfo } from "@/shared/pi-types";
import { mergeSessionStats, type SessionFileStats } from "@/features/sessions/session-stats";
import { userMessageKey } from "@/features/chat/prompt-recovery";
import { AgentEventConnection } from "@/features/chat/agent-event-connection";
import { getToolExecutionProgress } from "@/features/chat/tool-execution-progress";
import {
...
} from "@/features/chat/streaming-message";
...
export function useAgentSession(opts: UseAgentSessionOptions) {
  ...
  const loadTools = useCallback(async (sid: string) => {
    try {
      ...
      const { getPresetFromTools } = await import("@/features/chat/tool-presets");
