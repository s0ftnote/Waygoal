...
const chatInputSource = await readFile(new URL("../chat/ChatInput.tsx", import.meta.url), "utf8");
...
test("reuses the ChatInput model selector with scoped models", () => {
  ...
  assert.match(chatInputSource, /import \{ ModelSelector, type ModelSelectorOption \} from "\.\.\/settings\/ModelSelector"/);
