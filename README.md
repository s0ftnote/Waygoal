...
- 回看与继续：回看走只读历史读取，不调用会切换活动路径的树导航，也不新增消息；「从这里继续」才移动活动路径，下一次发送进入所选路径。查看位置和继续位置在界面上分开显示，刷新和宿主重启后都保留；记录里的位置找不到时明确提示，不按相似标题绑到另一段历史。
