...
const { getHelpText, parseLaunchOptions } = require("../../scripts/waygoal-options.js");
const cliPath = fileURLToPath(new URL("../../scripts/waygoal.js", import.meta.url));
...
test("supports --help and -h without starting the server", () => {
  ...
  assert.match(getHelpText(), /Usage: waygoal/);
...
test("CLI writes help and parse errors before exiting", () => {
  ...
  assert.match(help.stdout, /Usage: waygoal/);
...
test("rejects port values that could inject cmd arguments", () => {
  assert.throws(
    () => parseLaunchOptions(["-p", "30142&whoami"], {}),
    /Port must be a non-negative integer/,
  );
  assert.throws(
    () => parseLaunchOptions([], { PORT: "30142&whoami" }),
    /Port must be a non-negative integer/,
  );
});
