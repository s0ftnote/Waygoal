...
try {
  ...
  const openLinkTool = async () => {
    ...
    await page.locator('.waygoal-turn-more-body').getByRole('button', { name: /^关联讨论/ }).click();
  };
  ...
  await page.getByRole('button', { name: '全景', exact: true }).click();
  // Read the source at a useful zoom without opening or switching its chat.
  await source.hover();
  for (let i = 0; i < 14; i++) await page.mouse.wheel(0, -100);
  await delay(300);
  await source.hover();
  const sourceCard = page.locator(`[data-session="${id}"][data-turn="00000001"]`);
  await page.screenshot({ path: join(evidence, 'association-entry.png') });
  await sourceCard.getByRole('button', { name: /^关联讨论：/ }).click();
  check('direct card association preselects its source in one click', await sourceCard.getAttribute('data-link-source') === 'true');
  await page.keyboard.press('Escape');
  await sourceCard.getByRole('button', { name: /^关联讨论：/ }).focus();
  await page.keyboard.press('Enter');
  check('the direct association entry works from the keyboard without opening another chat', await sourceCard.getAttribute('data-link-source') === 'true' && await panel.getAttribute('data-session-id') === otherId);
  await page.getByRole('button', { name: '全景', exact: true }).click();
  check('selecting a link source keeps the current chat and explains the next step', await panel.getAttribute('data-session-id') === otherId && await page.getByText('点另一轮，和这轮一起回看', { exact: true }).isVisible());
  await page.screenshot({ path: join(evidence, 'association-desktop.png') });
  await targetCard.click();
  await page.getByText('已关联，可一起回看', { exact: true }).waitFor();
  ...
  await page.getByText('已关联，可一起回看', { exact: true }).waitFor({ state: 'hidden', timeout: 6000 });
  check('success feedback clears without an extra dismissal', await composer.inputValue() === '保留这段草稿');
  ...
  await targetCard.click(); await page.getByText('已关联，可一起回看', { exact: true }).waitFor();
  ...
  await openLinkTool(); await source.click();
  await page.keyboard.press('Escape');
  check('Escape cancels association without closing the chat', await panel.isVisible() && !await page.getByRole('group', { name: /^关联讨论/ }).count());
  await openLinkTool(); await source.click();
  await page.setViewportSize({ width: 390, height: 844 });
  await page.getByRole('group', { name: /^关联讨论/ }).waitFor();
  const guide = await page.getByRole('group', { name: /^关联讨论/ }).boundingBox();
