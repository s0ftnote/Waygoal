...
try {
  ...
  for (const viewport of [{ width: 1280, height: 800 }, { width: 390, height: 844 }]) {
    ...
    // A cold dev server compiles the page and session routes on first use.
    // Await both promises together so a timeout still reaches fixture cleanup.
    const [stateReady] = await Promise.all([
      page.waitForResponse((response) => new URL(response.url()).pathname === `/api/sessions/${LONG}/state`, { timeout: 120_000 }),
      page.goto(`${base}/chat?session=${LONG}`, { waitUntil: "domcontentloaded" }),
    ]);
    assert.equal(stateReady.status(), 200);
    ...
    await page.goto(`${base}/chat?session=${BRANCH}`, { waitUntil: "domcontentloaded" });
    ...
    await page.goto(`${base}/chat?session=${RICH}`, { waitUntil: "domcontentloaded" });
    ...
    await page.goto(`${base}/chat?session=${COMPACTED}`, { waitUntil: "domcontentloaded" });
